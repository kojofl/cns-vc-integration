...
export class RelationshipAttributeCreationHints extends Serializable implements IRelationshipAttributeCreationHints {
    ...
    @validate({
        customValidator: (v) =>
            !AttributeValues.Relationship.TYPE_NAMES.includes(v)
                ? `must be one of: ${AttributeValues.Relationship.TYPE_NAMES_STRINGIFIED}`
                : undefined
    })
    ...
    public static from(
        value: IRelationshipAttributeCreationHints | Omit<RelationshipAttributeCreationHintsJSON, "@type">
