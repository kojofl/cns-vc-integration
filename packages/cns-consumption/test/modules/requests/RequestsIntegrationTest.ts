...
export class RequestsTestsContext {
    ...
    public static async create(
        ...
        context.outgoingRequestsController = new OutgoingRequestsController(
            ...
            {
                getActiveRelationshipToIdentity: () =>
                    Promise.resolve(context.relationshipToReturnFromGetActiveRelationshipToIdentity)
            }
        )
...
export class RequestsWhen {
    ...
    public async iCreateAnOutgoingRequestFromRelationshipCreationChangeWith(
        ...
        this.context.localRequestAfterAction =
            await this.context.outgoingRequestsController.createAndCompleteFromRelationshipTemplateResponse(
    ...
    public async iCreateAnOutgoingRequestFromMessageWith(
        params: Partial<ICreateAndCompleteOutgoingRequestFromRelationshipTemplateResponseParameters>
    ): Promise<void> {
        params.template ??= TestObjectFactory.createOutgoingIRelationshipTemplate(
            this.context.currentIdentity,
            RelationshipTemplateContent.from({ onNewRelationship: TestObjectFactory.createRequestWithOneItem() })
        )
        params.responseSource ??= TestObjectFactory.createIncomingIMessageWithResponse(
    ...
    public async iCompleteTheOutgoingRequestWith(params: {
        ...
        this.context.localRequestAfterAction = await this.context.outgoingRequestsController.complete(
            params as ICompleteOutgoingRequestParameters
        )
    }
    ...
    public iTryToCreateAnOutgoingRequestFromRelationshipTemplateResponseWithoutResponseSource(): Promise<void> {
        const paramsWithoutResponseSource: Omit<
            ICreateAndCompleteOutgoingRequestFromRelationshipTemplateResponseParameters,
            "responseSource"
        > = {
        ...
        this.context.actionToTry = async () => {
            await this.context.outgoingRequestsController.createAndCompleteFromRelationshipTemplateResponse(
                paramsWithoutResponseSource as any
            )
        }
...
export class RequestsThen {
    ...
    public theRequestHasCorrectItemCount(itemCount: number): Promise<void> {
        expect(this.context.localRequestAfterAction?.content.items).to.has.lengthOf(itemCount)
    ...
    public theOnlyReturnedRequestIsInStatus(status: LocalRequestStatus): Promise<void> {
        expect(this.context.localRequestsAfterAction).to.have.lengthOf(1)
        expect(this.context.localRequestsAfterAction![0].status).to.equal(status)
        return Promise.resolve()
    }
