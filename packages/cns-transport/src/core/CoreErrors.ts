...
class Messages {
    ...
    public noMatchingRelationship(address: string) {
        return new CoreError(
            ...
            `A Relationship with the given address '${address}' does not exist.`
        )
    }
}
...
class Challenges {
    public challengeTypeRequiresRelationship() {
        return new CoreError(
            "error.transport.challenges.challengeTypeRequiresRelationship",
            "The challenge type 'Relationship' requires a relationship."
        )
    }
}
...
class Files {
    ...
    public invalidMetadata(id: string) {
        return new CoreError(
            "error.transport.files.invalidMetadata",
            `The metadata of the file with id "${id}" is invalid.`
        )
    }
...
class General {
    ...
    public platformClientSecretNotSet() {
        return new CoreError(
            "error.transport.general.platformClientSecretNotSet",
            "The platform clientSecret was not set."
        )
    }
