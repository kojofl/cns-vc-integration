...
export function AddUploadFile<TBase extends ConnectorTUIBaseConstructor>(
  Base: TBase
) {
  return class Sync extends Base {
    private readonly assetFolder = path.resolve(
    ...
    protected async uploadFile() {
      ...
      const expiresAt = DateTime.local().plus({ days: 1 }).toISO() as string;
      ...
      const file = await fs.promises.readFile(
        path.resolve(this.assetFolder, filename)
      );

      const uploadedFile = await this.connectorClient.files.uploadOwnFile({
        title,
        expiresAt,
        file,
        filename,
