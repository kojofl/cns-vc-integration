...
beforeAll(async () => {
    ...
    const senderAttribute = await sConsumptionServices.attributes.createAttribute({
        content: {
...
describe("ShareAttributeRequestItemDVO", () => {
    ...
    test("check the attributes for the sender", async () => {
        const dvo = (await sExpander.expandMessageDTO(senderMessage)) as RequestMessageDVO;
        const attributeResult = await sConsumptionServices.attributes.getSharedToPeerAttributes({
