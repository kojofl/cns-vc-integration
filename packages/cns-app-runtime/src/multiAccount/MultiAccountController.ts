...
export class MultiAccountController {
    ...
    public constructor(transport: Transport, private readonly config: AppConfig) {
    ...
    public async onboardDevice(deviceSharedSecret: DeviceSharedSecret): Promise<[LocalAccount, AccountController]> {
        this._log.trace(
            `Onboarding device ${deviceSharedSecret.id} for identity ${deviceSharedSecret.identity.address}...`
        )
