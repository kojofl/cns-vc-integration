...
export class AppStringProcessor {
    ...
    public async processTruncatedReference(
        ...
        if (
            truncatedReference.startsWith(Base64ForIdPrefix.File) ||
            truncatedReference.startsWith(Base64ForIdPrefix.RelationshipTemplate)
        ) {
    ...
    private async _handleTruncatedReference(
        truncatedReference: string,
        account: LocalAccountDTO
    ): Promise<UserfriendlyResult<void>> {
            ...
            if (result.error.code === "error.runtime.validation.invalidPropertyValue") {
                return UserfriendlyResult.fail(
                    new UserfriendlyApplicationError(
                        ...
                        "The given code does not contain a valid truncated reference."
                    )
                )
            }
        ...
        switch (result.value.type) {
            ...
            case "Token":
                return UserfriendlyResult.fail(
                    new UserfriendlyApplicationError(
                        "error.appStringProcessor.notSupportedTokenContent",
                        "The scanned code is not supported in this context"
                    )
                )
